...
# Restores APT configuration and keyrings from environment.tgz in the current directory.

set -euo pipefail
IFS=$'\n\t'

ARCHIVE="./environment.tgz"

if [ ! -f "$ARCHIVE" ]; then
  echo "[ERROR] environment.tgz not found in current directory!"
  exit 1
fi
